  ...
  data.forEach(playlist => {
    $('.fetch-data').append(`
      <div class="listed-playlist">
        <p class="name">
          <a class="playlist-select" href="../playlist_show/playlist_show.html?id=${playlist.id}">
            ${playlist.playlist_name}
          </a>
        </p>
        <p class="song_count">Song Count: ${playlist.songs.length - 1}</p>
      </div>
    `);
  })
}
...
const addToPlaylistsDisplay = (data) => {
  ...
  $('.fetch-data').append(`
    <div class="listed-playlist">
      <p class="name">
        <a href="../playlist_show/playlist_show.html?id=${playlist.id}">
          ${playlist.playlist_name}
        </a>
      </p>
